...
| --- | --- | --- |
| `MAGNETO_AUTH` | `none` | `none` for one implicit user, `proxy` to read the identity from a forward-auth proxy. In `proxy` a missing `Remote-User` is a 401, never a fallback |
| `MAGNETO_ADMIN_GROUP` | `admin` | Group name granting admin, matched against `Remote-Groups`. Ignored when `MAGNETO_AUTH=none`, where the only user is admin |
| `MAGNETO_LOGOUT_URL` | empty | Where the "Sign out" link points. The session belongs to the proxy, so this is its logout URL; empty hides the link |
| `MAGNETO_RETENTION` | `86400` | Seconds a downloaded file is kept, counted from the file's date. Its entry stays afterwards |
| `MAGNETO_DOWNLOAD_TIMEOUT` | `1200` | Seconds a single download may take before it is killed |
| `MAGNETO_PLAYLIST_MAX` | `50` | Most videos one pasted playlist may expand to |
| `MAGNETO_FEED` | `0` | The feed page, and with it the only thread that asks YouTube anything on its own. Off unless asked for |
| `MAGNETO_SHORTS` | `0` | The shorts page. Shorts come from the channels followed on the feed, so this does nothing while `MAGNETO_FEED` is off |
| `MAGNETO_TIKTOK` | `0` | The TikTok page: its own accounts, its own tab, the same player as the shorts page. Independent of `MAGNETO_FEED` and `MAGNETO_SHORTS` |
| `MAGNETO_SHARE` | `0` | Public share links. Off unless asked for, and it does nothing until the reverse proxy stops asking for authentication on `/s/` |
| `MAGNETO_SHARE_TTL` | `172800` | Seconds a share link works. The link is the whole credential and can be forwarded, so this is the real limit on who ends up watching |
| `MAGNETO_SHARE_MAX` | `10` | Live links one account may hold at a time. Expired ones do not count |
| `MAGNETO_SHORTS_RETENTION` | `86400` | Seconds a fetched short or TikTok clip is kept. A watched clip leaves the reel, so what this holds is one person's day of watching rather than everything their accounts list. Never pinned, and out of the history ceiling |
| `MAGNETO_SEEN_RETENTION` | `604800` | Seconds a "watched" row is kept. Past the point where the clip has left every listing, the row can no longer hide anything |
| `MAGNETO_FEED_VIDEOS` | `5` | Ceiling for the per-user feed setting, not the setting itself |
| `MAGNETO_FEED_POLL` | `300` | Seconds between two YouTube lookups. Each platform keeps its own clock, its own lock and its own queue: spacing exists to avoid being refused by a provider, and a provider only sees its own traffic |
| `MAGNETO_TIKTOK_POLL` | same as above | Seconds between two TikTok lookups. The pollers run side by side, so a provider that tolerates being asked more often can be, without spending anybody else's patience |
| `MAGNETO_FEED_TTL` | `21600` | Age at which a cached channel is worth looking up again |
| `MAGNETO_FEED_COOLDOWN` | `600` | Minimum age before a manual refresh does anything |
| `MAGNETO_FEED_CHANNELS_MAX` | `30` | YouTube channels one account may follow |
...
| `MAGNETO_NO_UPDATE` | unset | Set to any value to skip the yt-dlp update at container start. There is no good reason to: extractors break every few weeks and the update is the fix |
| `HOST` / `PORT` | `127.0.0.1` / `8899` | Only used by `python app.py` and `reclip.sh`. The image goes through gunicorn and reads `MAGNETO_BIND` |
...
Admin is membership of `MAGNETO_ADMIN_GROUP` in `Remote-Groups`. With
`MAGNETO_AUTH=none` the single user is admin, since there is nobody else.

## Accounts

With `MAGNETO_AUTH=none`, the default, everything belongs to one implicit user and
...
oauth2-proxy) and set `MAGNETO_AUTH=proxy`. The identity is read from
...
> On one host, bind to loopback with `MAGNETO_BIND=127.0.0.1:8899` and the rule
...
with `MAGNETO_AUTH=proxy` and `MAGNETO_BIND=127.0.0.1:8899` on the container.
...
every `MAGNETO_FEED_POLL`, and only if its copy is older than `MAGNETO_FEED_TTL`.
A manual refresh obeys the same spacing and reports channels looked up less than
`MAGNETO_FEED_COOLDOWN` ago as already fresh rather than as failures.
...
instance can be refused for its neighbours, and `MAGNETO_FORCE_IPV4` is on for
