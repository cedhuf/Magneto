...
while you watch, and both are swept after `RECLIP_SHORTS_RETENTION`. They stay
...
phone and a laptop agree, and forgotten after `RECLIP_SEEN_RETENTION`.
...
Off by default. With `RECLIP_SHARE=1` a clip gets a share button that mints a
...
- The link can be forwarded, so a share is public for as long as it lives.
  `RECLIP_SHARE_TTL` is the real control.
...
- `RECLIP_SHARE_MAX` bounds how many links one account holds at once.
